...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Trade rating endpoint
  app.post("/api/trades/:id/rating", authenticateToken, async (req, res) => {
    try {
      const tradeId = parseInt(req.params.id);
      const { ratedUserId, rating, comment, categories } = req.body;

      if (!ratedUserId || !rating || rating < 1 || rating > 5) {
        return res.status(400).json({ error: "Valid rated user ID and rating (1-5) are required" });
      }
      ...
      if (trade.buyerId !== req.user!.id && trade.sellerId !== req.user!.id) {
        return res.status(403).json({ error: "Unauthorized to rate this trade" });
      }
      ...
      const expectedRatedUserId = trade.buyerId === req.user!.id ? trade.sellerId : trade.buyerId;
      if (ratedUserId !== expectedRatedUserId) {
        return res.status(400).json({ error: "Can only rate your trading partner" });
      }

      const existingRating = await storage.getTradeRating(tradeId, req.user!.id);
      if (existingRating) {
      ...
      await storage.createNotification({
        ...
        message: `You received a ${rating}-star rating for Trade #${tradeId}`,
        data: JSON.stringify({ tradeId, rating, comment })
      });
  ...
  // Get user profile with ratings summary
  app.get("/api/users/:userId/profile", authenticateToken, async (req, res) => {
    try {
      ...
      const profile = {
        ...
        completedTrades,
        disputedTrades,
        successRate: trades.length > 0 ? ((completedTrades / trades.length) * 100).toFixed(1) : '0',
